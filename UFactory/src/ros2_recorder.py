...
class _DobotRecorderNode(Node):
    ...
    def __init__(self):
        super().__init__('xarm_recorder')
        self._bridge = CvBridge()
        self._lock   = threading.Lock()

        # ── Publishers ─────────────────────────────────────────────────────
        self._pub_hik    = self.create_publisher(Image,             '/xarm/hik/image_raw', 5)
        self._pub_zed    = self.create_publisher(Image,             '/xarm/zed/image_raw', 5)
        self._pub_joints = self.create_publisher(JointState,        '/xarm/joint_states',  5)
        self._pub_tcp    = self.create_publisher(Float32MultiArray, '/xarm/tcp_pose',       5)
        self._pub_mode   = self.create_publisher(Int32,             '/xarm/robot_mode',     5)
        ...
        sub_hik = message_filters.Subscriber(self, Image, '/xarm/hik/image_raw')
        sub_zed = message_filters.Subscriber(self, Image, '/xarm/zed/image_raw')
